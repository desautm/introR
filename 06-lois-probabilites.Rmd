...
Pour la loi binomiale, ces fonctions sont `pbinom`, `qbinom`, `dbinom` et `rbinom`.
...
ggplot(fpois, aes(x = x, y = y)) +
  ...
  labs(
    ...
    y = "Probabilité",
    title = "Répartition de la probabilité de la loi de Poisson en fonction du nombre d'e succès d'événements"
  )
```
...
|-----------:|:-----------|
|$P(X=k)$    |`dgeom(k, p)`|
|$P(i\leq X \leq j)$ | `sum(dpgeom(i:j, p))`|
|$P(X\leq k)$ | `pgeom(k, p)` |
|$P(X>k)$ | `1-pgeom(k, p)` |
